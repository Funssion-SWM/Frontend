...
export default function CountInfo({ userId }: Props) {
  ...
  const onClickUnfollow = (userId: string) => {
    ...
    updateFollows(
      follows.filter((follow) => follow.userId.toString() != userId)
    );
  };
  ...
  return (
    <>
      <div className="flex text-center w-full border-y">
        ...
        >
          <p className="text-lg font-semibold">{followCnt}</p>
          <p className="text-xs text-gray-400">팔로우</p>
        </div>
        <div
          className="w-1/3 cursor-pointer transition-all hover:bg-soma-grey-30 py-4"
          onClick={() =>
        ...
        >
          <p className="text-lg font-semibold">{followerCnt}</p>
